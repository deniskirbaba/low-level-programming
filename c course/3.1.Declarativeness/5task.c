...
enum opcode {BC_PUSH, BC_IPRINT, BC_IREAD, BC_IADD, BC_STOP, BC_ISUB, BC_IMUL, BC_IDIV, BC_IMOD, BC_INEG, BC_SWAP,
        BC_POP, BC_DUP, BC_ICMP, BC_JMP, BC_JZ};
...
void interpret(struct vm_state* state, ins_interpreter * const  (actions)[]) {
    for (; state->ip ;) {
        ...
        if ((int64_t)ins_descr->stack_delta + (int64_t)state->data_stack.count > (int64_t)state->data_stack.data.size) {
